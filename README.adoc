...
Annotate the `SystemService` class with [hotspot=serverEndpoint file=0]`@ServerEndpoint` annotation to make it as a WebSocket server. The methods define the lifecycle of different phases like establishing a connection, receiving a message, and closing the connection with the [hotspot=onOpen file=0]`@OnOpen`, [hotspot=onMessage file=0]`@OnMessage` and [hotspot=onClose file=0]`@OnClose` annotations respectively. The method annotated with the [hotspot=onError file=0]`@OnError` annotation that is responsible for tackling errors.

On the `onMessage()` method, when a message is received through the parameter `option`, construct a message with different values and then boardcast the message to the client.
