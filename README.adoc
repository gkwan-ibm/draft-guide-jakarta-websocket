...
Most modern web browsers support Jakarta Web Sockets. The onopen and onerror functions are responsible for logging the events in the console where as the onmessage event is responsible for adding a row when the loadAverage and the memoryUsage is sent from the server. 
