...
:page-description: Learn how to stream messages between server service and front-end clients by using Jakarta WebSockets.
:guide-author: Open Liberty
:page-tags: ['Jakarta EE']
:page-related-guides: ['grpc-intro']
:page-permalink: /guides/{projectid}
:imagesdir: /img/guide/{projectid}
:page-seo-title: Streaming messages to communicate server service and front-end clients using Jakarta WebSockets APIs
:page-seo-description: A getting started tutorial with examples on how to stream messages to communicate front-end clients and a cloud-native Java application by using Jakarta WebSockets APIs.
:common-includes: https://raw.githubusercontent.com/OpenLiberty/guides-common/prod
:source-highlighter: prettify
= Streaming messages between server service and clients using Jakarta WebSockets

[.hidden]
NOTE: This repository contains the guide documentation source. To view the guide in published form, view it on the https://openliberty.io/guides/{projectid}.html[Open Liberty website].

Learn how to stream messages between a server service and front-end clients by using Jakarta WebSockets.
...
https://jakarta.ee/specifications/websocket[Jakarta WebSockets^] enable dual communication between server and client endpoints. First, the clients make connection over to the server using HTTP. The server can boardcast messages to the clients. WebSockets differ from Server-Sent Events (SSE) that SSE is unidirectional from server to client whereas WebSockets are bidirectional. WebSockets also enable the functionality of real-time updates over a smaller bandwidth.

The application you will build in this guide consists of a `system` server service and a `frontend` client service. The scheduler in the `frontend` service sends messages to the `system` service every 10 seconds. The `system` service broadcasts messages to the front-end clients. The following diagram depicts the application that is used in this guide. 
