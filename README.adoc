...
The [hotspot=SystemLoadEncoder file=1]`SystemLoadEncoder` class implements the `Encoder.Text` interface, accepts the `JsonObject` message, and converts the message to a string value by the [hotspot=encode file=1]`encode()` override method.
...
The [hotspot=SystemLoadDecoder file=2]`SystemLoadDecoder` class implements the `Decoder.Text` interface, accepts string message, and decodes the string back into a `JsonObject` by the [hotspot=encode file=1]`decode()` override method. The [hotspot=willDecode file=2]`willDecode` override method checks whether the string can be decoded into JSON object and returns a boolean value.
