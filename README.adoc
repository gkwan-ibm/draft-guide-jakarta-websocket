...
The [hotspot=onMessageMethod file=0]`onMessage()` method is called when a message is received through the parameter `option`. According to the [hotspot=loadAverage hotspot=memoryUsageOrBoth file=0]`option` value, construct a message with different values and boardcast the `JsonObject` message to the clients by the [hotspot=sendToAllSessions file=0]`sendToAllSessions()` method. The [hotspot=sendToAllSessionseMethod file=0]`sendToAllSessions()` method uses the Websockets API to boardcast the message to all client sessions.
