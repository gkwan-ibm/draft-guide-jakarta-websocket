...
The `SystemClient` class is similar to the WebSocket client class from the `frontend` service created in the previous section. At the [hotspot=onMessage file=0]`onMessage()` method, decode and verify the message . 
...
There are two test cases to ensure correct functionality of the `system` sevice. The `testSystem()` method verifies one client connention and the `testSystemMultipleSessions()` method verifies multiple client connentions. 
