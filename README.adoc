...
This class decodes the String back into a JSON object. This is achieved using the [hotspot=decode file=2]`encode' method. It also adds a [hotspot=willDecode file=2]`willDecode` check to see if the String can be decoded into JSON object and returns a boolean value of either true or false.
