...
In this section, you will create the WebSocket `system` service that broadcasts messages to the clients.
...
The `system` service is responsible for handling the messages produced by the scheduler, building system load messages, and forwarding them to the clients. This can be achieved by configuring the `system` service as a WebSocket `ServerEndpoint`.
