...
In this section, you will create the WebSocket client that communicate with the WebSocket server, the scheduder that uses the WebSocket client to send messages to the server, and a HTML file that uses JavaScrpt `WebSocket` object to build websocket connenction, subscribe on different events, and display the broadcasting messages from the `system` service in a table.
...
Annotate the [hotspot=clientEndpoint file=0]`SystemClient` class with [hotspot=clientEndpoint file=0]`@ClientEndpoint` annotation to make it as a WebSocket client. Create a constructor that uses the [hotspot=webSocketAPI file=0]`websocket` APIs to establish connection with the server. Annotate the `onOpen()` method with the [hotspot=onOpen file=0]`@OnOpen` annotation that persists the client session when the connection is established. Annotate the `onMessage()` method with the [hotspot=onMessage file=0]`@OnMessage` annotation that handles messages from the `system` WebSocket server.
